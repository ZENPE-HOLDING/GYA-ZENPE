...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      gyaio::client::http::connection_param *cp = new gyaio::client::http::connection_param(context, parse_url(url) + path,
              no_verify ? false : true, headers);

      return gyaio::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
...
void ensure_kgyad_running(CLI::App* app) {
    ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("kgyad"); // if clgya and kgyad are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kgyad").append("kgyad");
    }
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      GYA_ASSERT( !wast.empty(), wast_file_not_found, "no wast file found ${f}", ("f", wastPath) );
      vector<uint8_t> wasm;
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "gyaio.msig", "propose", result.get_object()["binargs"].as<bytes>()}});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "gyaio.msig", action, result.get_object()["binargs"].as<bytes>()}});
   };
   ...
   cancel->set_callback([&]() {
      ...
      auto result = call(json_to_bin_func, arg);
      send_actions({chain::action{accountPermissions, "gyaio.msig", "cancel", result.get_object()["binargs"].as<bytes>()}});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "gyaio.msig", "exec", result.get_object()["binargs"].as<bytes>()}});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      auto result = call(json_to_bin_func, arg);
      send_actions({chain::action{accountPermissions, "gyaio.sudo", "exec", result.get_object()["binargs"].as<bytes>()}});
   });
