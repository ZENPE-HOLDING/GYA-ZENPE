...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      gyaio::client::http::connection_param *cp = new gyaio::client::http::connection_param((std::string&)url, (std::string&)path,
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "GYA")) {
      try {
...
void ensure_kgyad_running(CLI::App* app) {
    ...
    if (port_used(uint16_t(wallet_port)))  // Hopefully taken by kgyad
        return;
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of GYA to pay for RAM"))->required();
