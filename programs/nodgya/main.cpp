/**
 *  @file
 *  @copyright defined in gyaio/LICENSE.txt
 */
...
{
   try {
      app().set_version(gyaio::nodgya::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "gyaio/nodgya/data" );
      app().set_default_config_dir(root / "gyaio/nodgya/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodgya version ${ver}", ("ver", gyaio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("gyaio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
