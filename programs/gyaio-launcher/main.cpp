...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("nodgya", bpo::value<string>(&gyad_extra_args), "forward nodgya command line argument(s) to each instance of nodgya, enclose arg in quotes")
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "cd " + host.gyaio_home + "; "
                 + "export GYAIO_HOME=" + host.gyaio_home + string("; ")
                 + "export GYAIO_NODE=" + node_num + "; "
                 + "./scripts/gyaio-tn_bounce.sh " + gyad_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.gyaio_home + "; "
                 + "export GYAIO_HOME=" + host.gyaio_home + "; "
                 + "export GYAIO_NODE=" + node_num + "; "
         + "export GYAIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/gyaio-tn_down.sh";
