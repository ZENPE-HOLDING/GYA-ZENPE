...
#include <net/if.h>
#include <gyaio/chain/genesis_state.hpp>
...
void
gyad_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string gyaio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), gyaio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  string gyad_extra_args;
  std::map<uint,string> specific_nodgya_args;
  testnet_def network;
  string gelf_endpoint;
  ...
  producer_set_def producer_set;
   ...
   void assign_name (gyad_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (gyad_def &node, host_def *host);
  void launch (gyad_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  static string get_node_num(uint16_t node_num);
  pair<host_def, gyad_def> find_node(uint16_t node_num);
  vector<pair<host_def, gyad_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodgya does not require transaction signatures.")
    ("nodgya", bpo::value<string>(&gyad_extra_args), "forward nodgya command line argument(s) to each instance of nodgya, enclose arg(s) in quotes")
    ("specific-num", bpo::value<vector<uint>>()->composing(), "forward nodgya command line argument(s) (using \"--specific-nodgya\" flag) to this specific instance of nodgya. This parameter can be entered multiple times and requires a paired \"--specific-nodgya\" flag")
    ("specific-nodgya", bpo::value<vector<string>>()->composing(), "forward nodgya command line argument(s) to its paired specific instance of nodgya(using \"--specific-num\"), enclose arg(s) in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodgya instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-nodgya"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-nodgya argument" << endl;
      exit (-1);
    }
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodgya_args[num] = specific_args[i];
    }
  }
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &gyad : binding.instances) {
          gyad.host = binding.host_name;
          gyad.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(gyad.p2p_port);

          aliases.push_back (gyad.name);
        }
      }
  ...
  char* erd_env_var = getenv ("GYAIO_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable GYAIO_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (gyad_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.gyaio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      gyad_def gyad;
      assign_name(gyad, i == 0);
      aliases.push_back(gyad.name);
      gyad.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(gyad));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->gyaio_home =
          (local_id.contains (lhost->host_name) || servers.default_gyaio_home.empty()) ?
          erd : servers.default_gyaio_home;
        host_ndx++;
      } // ph_count == 0

      gyad_def gyad;
      assign_name(gyad, do_bios);
      gyad.has_db = false;
      ...
      aliases.push_back(gyad.name);
      gyad.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(gyad));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  gyad_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->gyaio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->gyaio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->gyaio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->gyaio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->gyaio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = gyaio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = gyaio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = gyaio::net_plugin\n";
  } else {
    cfg << "plugin = gyaio::bnet_plugin\n";
  }
  cfg << "plugin = gyaio::chain_api_plugin\n"
      << "plugin = gyaio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      gyaio::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (gyad_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->gyaio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->gyaio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->gyaio_home;

  cmd = "cd " + host->gyaio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->gyaio_home
...
void
launcher_def::launch (gyad_def &instance, string &gts) {
  ...
  string gyadcmd = "programs/nodgya/nodgya ";
  if (skip_transaction_signatures) {
    gyadcmd += "--skip-transaction-signatures ";
  }
  if (!gyad_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +gyaio::mongo_db_plugin");
       string args = std::regex_replace (gyad_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       gyadcmd += args + " ";
    }
    else {
       gyadcmd += gyad_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodgya_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodgya_args.count(node_num)) {
        gyadcmd += specific_nodgya_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    gyadcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  gyadcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  gyadcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    gyadcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->gyaio_home + "; nohup " + gyadcmd + " > "
    ...
    string cmd = "cd " + host->gyaio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << gyadcmd << endl;

    bp::child c(gyadcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << gyadcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(gyad_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, gyad_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, gyad_def>> node_list;
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.gyaio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const gyad_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/gyaio-tn_bounce.sh " + gyad_extra_args;
      do_command(host, node.name, { { "GYAIO_HOME", host.gyaio_home }, { "GYAIO_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/gyaio-tn_down.sh ";
      do_command(host, node.name,
                 { { "GYAIO_HOME", host.gyaio_home }, { "GYAIO_NODE", node_num }, { "GYAIO_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/gyaio-tn_roll.sh ";
      do_command(*host, host_name, { { "GYAIO_HOME", host->gyaio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the gyaio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the gyaio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the gyaio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << gyaio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(gyaio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (gyaio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_gyaio_home) (ssh) )
