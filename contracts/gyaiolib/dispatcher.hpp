...
namespace gyaio {
    ...
    * ```
    *
    * For this to work the Actions must be derived from gyaio::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return gyaio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from gyaio::contract
...
// Helper macro for GYAIO_API
#define GYAIO_API_CALL( r, OP, elem ) \
   case ::gyaio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      gyaio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for GYAIO_ABI
#define GYAIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( GYAIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from gyaio::contract
 * 
 ...
 * @endcode
 */
#define GYAIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "gyaio" code account and authorized by "gyaio"'s "active permission */ \
         gyaio_assert(code == N(gyaio), "onerror action's are only valid from the \"gyaio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            GYAIO_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: gyaio_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
