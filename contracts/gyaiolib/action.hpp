...
namespace gyaio {
    ...
    *    int  c; //4
    *
    *    GYALIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      GYALIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      GYALIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         gyaio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         gyaio_assert( name == T::get_name(), "Invalid name" );
         gyaio_assert( account == T::get_account(), "Invalid account" );
...
::gyaio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::gyaio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::gyaio::action_meta<CODE, ::gyaio::string_to_name(#NAME) >
