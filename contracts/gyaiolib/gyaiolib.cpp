...
namespace gyaio {
   ...
   void set_blockchain_parameters(const gyaio::blockchain_parameters& params) {
      char buf[sizeof(gyaio::blockchain_parameters)];
      gyaio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(gyaio::blockchain_parameters& params) {
      char buf[sizeof(gyaio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      gyaio_assert( size <= sizeof(buf), "buffer is too small" );
      gyaio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            gyaio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return gyaio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = gyaio::memory_heap.malloc(count*size);
...
{
   return gyaio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return gyaio::memory_heap.free(ptr);
}
