...
class simpletoken : public gyaio::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         gyaio_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      gyaio::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              gyaio_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

GYAIO_ABI( simpletoken, (transfer)(issue) )
