...
Execute a transaction while bypassing regular authorization checks (requires authorization of gyaio.sudo which needs to be a privileged account).

### gyaio.sudo::exec    executer trx
...
## 2. Installing the gyaio.sudo contract

The gyaio.sudo contract needs to be installed on a privileged account to function. It is recommended to use the account `gyaio.sudo`.

First, the account `gyaio.sudo` needs to be created. Since it has the restricted `gyaio.` prefix, only a privileged account can create this account. So this guide will use the `gyaio` account to create the `gyaio.sudo` account. On typical live blockchain configurations, the `gyaio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `gyaio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `gyaio.sudo` account.

The `gyaio.sudo` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`gyaio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `gyaio` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `gyaio` account.

This guide will be using clgya to carry out the process.

### 2.1 Create the gyaio.sudo account

#### 2.1.1 Generate the transaction to create the gyaio.sudo account

The transaction to create the `gyaio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clgya command to propose the transaction to the gyaio.msig contract.

A simple way to generate a transaction to create a new account is to use the `clgya system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `gyaio`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `clgya set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using clgya and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the gyaio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ clgya system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 gyaio gyaio.sudo GYA8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"gyaio","action":"buyrambytes","args":{"payer":"gyaio","receiver":"gyaio.sudo","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"gyaio","action":"delegatebw","args":{"from":"gyaio","receiver":"gyaio.sudo","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
The actual public key used is not important since that data is only encoded into the `gyaio::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `gyaio::newaccount` action. It should look like:
```
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "gyaio", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
Third, generate a transaction containing the actual `gyaio::newaccount` action that will be used in the final transaction:
```
$ clgya push action -s -j -d gyaio newaccount newaccount_payload.json -p gyaio > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `gyaio::setpriv` action which will make the `gyaio.sudo` account privileged:
```
$ clgya push action -s -j -d gyaio setpriv '{"account": "gyaio.sudo", "is_priv": 1}' -p gyaio > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the gyaio.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_sudo_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the gyaio.msig contract.
...
#### 2.1.2 Propose the transaction to create the gyaio.sudo account
...
```
$ clgya multisig propose_trx createsudo producer_permissions.json create_sudo_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    gyaio.msig <= gyaio.msig::propose          {"proposer":"blkproducera","proposal_name":"createsudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the gyaio.sudo account
...
The proposed transaction can be reviewed using the `clgya multisig review` command:
```
$ clgya multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ clgya multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
```
$ clgya multisig approve blkproducera createsudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    gyaio.msig <= gyaio.msig::approve          {"proposer":"blkproducera","proposal_name":"createsudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the gyaio.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `gyaio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the gyaio.msig contract).

```
$ clgya multisig exec blkproducera createsudo blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    gyaio.msig <= gyaio.msig::exec             {"proposer":"blkproducera","proposal_name":"createsudo","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `gyaio.sudo` was created:
```
$ clgya get account gyaio.sudo
privileged: true
permissions:
     owner     1:    1 gyaio@active,
        active     1:    1 gyaio@active,
memory:
...
### 2.2 Deploy the gyaio.sudo contract

#### 2.2.1  Generate the transaction to deploy the gyaio.sudo contract

The transaction to deploy the contract to the `gyaio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clgya command to propose the transaction to the gyaio.msig contract.

The easy way to generate this transaction is using clgya:
```
$ clgya set contract -s -j -d gyaio.sudo contracts/gyaio.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/gyaio.sudo/gyaio.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the gyaio.sudo contract
...
```
$ clgya multisig propose_trx deploysudo producer_permissions.json deploy_sudo_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    gyaio.msig <= gyaio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploysudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the gyaio.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `clgya multisig review` command:
```
$ clgya multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ clgya multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clgya multisig approve blkproducera deploysudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    gyaio.msig <= gyaio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploysudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the gyaio.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `gyaio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the gyaio.msig contract).

```
$ clgya multisig exec blkproducera deploysudo blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    gyaio.msig <= gyaio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploysudo","executer":"blkproducera"}
```

Anyone can now verify that the `gyaio.sudo` contract was deployed correctly.

```
$ clgya get code -a retrieved-gyaio.sudo.abi gyaio.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-gyaio.sudo.abi
$ sha256sum contracts/gyaio.sudo/gyaio.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/gyaio.sudo/gyaio.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-gyaio.sudo.abi, can then be compared to the original ABI of the contract (contracts/gyaio.sudo/gyaio.sudo.abi) to ensure they are semantically the same.

## 3. Using the gyaio.sudo contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed gyaio.sudo contract together with the gyaio.msig contract to allow a greater than two-thirds supermajority of block producers of an GYAIO blockchain to change the owner authority of an arbitrary account. The example will use clgya: in particular, the `clgya multisig` command, the `clgya set account permission` sub-command, and the `clgya sudo exec` sub-command. However, the guide also demonstrates what to do if the `clgya sudo exec` sub-command is not available.
...
```
permissions:
     owner     1:    1 GYA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
        active     1:    1 GYA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
memory:
...
Assume that none of the block producers know the private key corresponding to the public key `GYA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```
$ clgya set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "gyaio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `gyaio.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ clgya sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `clgya sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `clgya sudo exec` sub-command method.
...
```
$ clgya multisig propose_trx -s -j -d nothing '[]' update_alice_owner_trx.json nothing | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > update_alice_owner_trx_serialized.hex
...
Then generate the template for the transaction containing the `gyaio.sudo::exec` action:
```
$ clgya push action -s -j -d gyaio.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
```
$ clgya multisig propose_trx updatealice producer_permissions.json sudo_update_alice_owner_trx.json blkproducera
executed transaction: 10474f52c9e3fc8e729469a577cd2fc9e4330e25b3fd402fc738ddde26605c13  624 bytes  782 us
#    gyaio.msig <= gyaio.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ clgya multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodgya/clgya, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `gyaio.sudo::exec` action is available in the output of the `clgya multisig review` command, only the hex data is available of the payload of the inner `gyaio::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodgya/clgya, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `gyaio::setcode` action to set the contract code of the `gyaio.sudo` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ clgya multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clgya multisig approve blkproducera updatealice '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 2bddc7747e0660ba26babf95035225895b134bfb2ede32ba0a2bb6091c7dab56  128 bytes  543 us
#    gyaio.msig <= gyaio.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
