...
## gyaio.system::cfgproducer    account config
...
## gyaio.system::okproducer     account producer vote
...
## gyaio.system::setproxy       account proxy
...
## gyaio.system::unstake        account quantity 
...
## gyaio.system::withdraw account
   - this action can only be triggered by gyaio.system via a deferred transaction generated by unstake
   - this will generate an inline gyaio.token::transfer call from=gyaio.system to=account and amount equal to the next withdraw increment
   - this will generate another deferred withdraw to continue the process if there are still withdraws to be made


## gyaio.system::transfer from to amount memo
   - decrements balance of from if amount <= balance
   - increments balance of to by amount
   - memo is ignored 

## gyaio.system::stakevote account amount
   - the primary currency of gyaio is controlled by the token contract which enables users to transfer
...
## gyaio.system::onblock   account blocktime blocknum
...
## gyaio.system::freeze producer accounts true|false 
...
## gyaio.system::paystandby producer
