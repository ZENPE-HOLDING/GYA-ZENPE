#include <gyaio.msig/gyaio.msig.hpp>
#include <gyaiolib/action.hpp>
#include <gyaiolib/permission.hpp>
...
void multisig::propose() {
   ...
   gyaio_assert( trx_header.expiration >= gyaio::time_point_sec(now()), "transaction expired" );
   //gyaio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   gyaio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   gyaio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   gyaio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   gyaio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      gyaio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < gyaio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
GYAIO_ABI( gyaio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
