...
namespace gyaio {
   ...
   struct margin_position {
      ...
      GYALIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef gyaio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), gyaio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      GYALIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef gyaio::multi_index<N(loans), loan_position> loans;
