#pragma once
#include <gyaio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( gyaio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(gyaio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(gyaio::chain::transaction_receipt, (gyaio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(gyaio::chain::signed_block, (gyaio::chain::signed_block_header), (transactions)(block_extensions) )
