...
#define N(X) gyaio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<gyaio::chain::name> : private hash<uint64_t> {
      typedef gyaio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const gyaio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, gyaio::chain::name& check);
} // fc


FC_REFLECT( gyaio::chain::name, (value) )
