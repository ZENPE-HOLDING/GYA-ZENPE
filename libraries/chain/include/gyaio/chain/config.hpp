...
namespace gyaio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(gyaio);
const static uint64_t null_account_name      = N(gyaio.null);
const static uint64_t producers_account_name = N(gyaio.prods);
...
const static uint64_t gyaio_auth_scope       = N(gyaio.auth);
const static uint64_t gyaio_all_scope        = N(gyaio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t gyaio_any_name = N(gyaio.any);
const static uint64_t gyaio_code_name = N(gyaio.code);
...
const static gyaio::chain::wasm_interface::vm_type default_wasm_runtime = gyaio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace gyaio::chain::config

constexpr uint64_t GYA_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / gyaio::chain::config::percent_100;
}

template<typename Number>
Number GYA_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + gyaio::chain::config::percent_100 - gyaio::chain::config::percent_1)  / gyaio::chain::config::percent_100;
}
