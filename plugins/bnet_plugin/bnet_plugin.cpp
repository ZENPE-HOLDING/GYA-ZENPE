...
#include <gyaio/bnet_plugin/bnet_plugin.hpp>
...
using gyaio::public_key_type;
using gyaio::chain_id_type;
using gyaio::block_id_type;
using gyaio::block_timestamp_type;
using std::string;
using gyaio::sha256;
using gyaio::signed_block_ptr;
using gyaio::packed_transaction_ptr;
using std::vector;
...
namespace gyaio {
     ...
     void on( const hello& hi, fc::datastream<const char*>& ds );
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              GYA_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              GYA_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           GYA_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         GYA_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
