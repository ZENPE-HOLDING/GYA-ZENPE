   ...
   try {
      ...
      if( account ) {
         ...
         try {
            ...
            ilog( "Unable to convert GYA JSON to MongoDB JSON: ${e}", ("e", e.what()));
            ilog( "  GYA JSON: ${j}", ("j", json));
      ...
      if (act.name != "onblock") { // gyaio::onblock not in original gyaio.system abi
      ...
      if (actions_to_write) {
         try {
            if( !bulk_actions.execute() ) {
               GYA_ASSERT( false, chain::mongo_db_insert_fail, "Bulk actions insert failed for transaction: ${id}", ("id", trx_id_str));
            }
         } catch(...) {
