...
public:
   ...
   static abi_def gya_abi; // cached for common use
                ...
                const chain::action& msg)
  {
     ...
     try {
        ...
        if (msg.account == chain::config::system_account_name) {
           abi = chain::gyaio_contract_abi(abi);
        }
        abis.set_abi(abi);
        ...
        try {
           ...
           elog("Unable to convert GYA JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("  GYA JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
// For now providing some simple account processing to maintain gya_balance
void mongo_db_plugin_impl::update_account(const chain::action& msg) {
   ...
   if (msg.name == transfer) {
      ...
      abi_serializer abis;
      auto gyaio_account = find_account(accounts, msg.account);
      auto abi = fc::json::from_string(bsoncxx::to_json(gyaio_account.view()["abi"].get_document())).as<abi_def>();
      abis.set_abi(abi);
      ...
      asset from_balance = asset::from_string(from_account.view()["gya_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["gya_balance"].get_utf8().value.to_string());
      ...
      update_from << "$set" << open_document << "gya_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "gya_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The queue size between nodgya and MongoDB plugin thread.")
