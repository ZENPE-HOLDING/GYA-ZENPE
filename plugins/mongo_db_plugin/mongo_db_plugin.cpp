   ...
   if( write_atraces ) {
      try {
         if( !bulk_action_traces.execute() ) {
            GYA_ASSERT( false, chain::mongo_db_insert_fail, "Bulk action traces insert failed for transaction trace: ${id}", ("id", t->id));
         }
      } catch(...) {
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.count( "mongodb-filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               GYA_ASSERT( fe.receiver.value && fe.action.value, fc::invalid_arg_exception,
         ...
         if( options.count( "mongodb-filter-out" )) {
            ...
            for( auto& s : fo ) {
               ...
               filter_entry fe{v[0], v[1], v[2]};
               GYA_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
